...
// Procesar formulario de actualización de género
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $nuevo_nombre = trim($_POST['genero']);
    if ($nuevo_nombre !== '') {
        $sqlUpdate = "UPDATE Generos SET genero = ? WHERE id_genero = ?";
        $stmtUpdate = $conn->prepare($sqlUpdate);
        $stmtUpdate->bind_param("si", $nuevo_nombre, $id_genero);
        if ($stmtUpdate->execute()) {
            // Recargar para mostrar cambios
            header("Location: detallegenero.php?id=" . $id_genero . "&msg=actualizado");
            exit();
        } else {
            $errorMsg = "Error al actualizar el género.";
        }
    } else {
        $errorMsg = "El nombre del género no puede estar vacío.";
    }
}

// Obtener datos del género
$sqlGenero = "SELECT * FROM Generos WHERE id_genero = ?";
...
$sqlLibros = "SELECT * FROM Libros WHERE genero = ? ORDER BY titulo ASC";
$stmtLibros = $conn->prepare($sqlLibros);
$stmtLibros->bind_param("s", $genero['genero']);
$stmtLibros->execute();
...
<title>Detalle Género - LIBRATEC</title>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.6.0/css/all.min.css" />
<style>
    /* Copia los estilos del navbar y tabla */
    * {
    ...
    form.update-genero input[type="text"] {
        width: 100%;
        padding: 10px 12px;
        font-size: 1.1rem;
        border-radius: 6px;
    ...
    form.update-genero button:hover {
...
<main class="main">
    ...
    <?php if (isset($_GET['msg']) && $_GET['msg'] === 'actualizado'): ?>
        <p class="success-message">Género actualizado correctamente.</p>
    <?php endif; ?>
    ...
    <form method="POST" class="update-genero" action="detallegenero.php?id=<?php echo $id_genero; ?>">
        <label for="genero"><strong>Nombre del Género:</strong></label>
        <input type="text" id="genero" name="genero" value="<?php echo htmlspecialchars($genero['genero']); ?>" required />
        <button type="submit">Actualizar Género</button>
    </form>

    <h2>Libros del Género "<?php echo htmlspecialchars($genero['genero']); ?>"</h2>
        ...
        <table>
            ...
            <tbody>
                <?php if ($resLibros && $resLibros->num_rows > 0): ?>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="10">Este género no tiene libros registrados.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
