...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($carrera === '') {
        ...
        // Insertar carrera
        $sqlInsert = "INSERT INTO Carreras (carrera) VALUES (?)";
        $stmtInsert = $conn->prepare($sqlInsert);
        $stmtInsert->bind_param("s", $carrera);

        if ($stmtInsert->execute()) {
            // Redirigir a carreras.php después de agregar
            header("Location: carreras.php");
            exit();
        } else {
            $mensaje = "Error al agregar la carrera. Inténtalo de nuevo.";
        }
    }
}
?>
...
<title>Añadir Carrera - LIBRATEC</title>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.6.0/css/all.min.css" />
<style>
    /* Aquí va el mismo CSS que el ejemplo, adaptado para mantener coherencia visual */
    * {
...
<main class="main">
    ...
    <form method="POST" action="añadircarrera.php" autocomplete="off">
        ...
        <input 
            ...
            required
            maxlength="100"
            value="<?php echo isset($_POST['carrera']) ? htmlspecialchars($_POST['carrera']) : ''; ?>"
        />

        <button type="submit" class="btn-submit"><i class="fa fa-save"></i> Guardar Carrera</button>
    </form>

    <a href="carreras.php" class="btn-cancel">Cancelar</a>
</main>
