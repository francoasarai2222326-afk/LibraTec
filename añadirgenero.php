...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($genero === '') {
        $mensaje = "El nombre del género no puede estar vacío.";
    } else {
        // Insertar género
        $sqlInsert = "INSERT INTO Generos (genero) VALUES (?)";
        $stmtInsert = $conn->prepare($sqlInsert);
        $stmtInsert->bind_param("s", $genero);

        if ($stmtInsert->execute()) {
            // Redirigir a generos.php después de agregar
            header("Location: generos.php");
            exit();
        } else {
            $mensaje = "Error al agregar el género. Inténtalo de nuevo.";
        }
    }
}
?>
...
<title>Añadir Género - LIBRATEC</title>
...
<main class="main">
    ...
    <form method="POST" action="añadirgenero.php" autocomplete="off">
        ...
        <input 
            ...
            required
            maxlength="100"
            value="<?php echo isset($_POST['genero']) ? htmlspecialchars($_POST['genero']) : ''; ?>"
        />

        <button type="submit" class="btn-submit"><i class="fa fa-save"></i> Guardar Género</button>
    </form>
