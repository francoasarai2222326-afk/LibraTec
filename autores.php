...
// Consulta autores con filtro búsqueda
if ($busqueda !== "") {
    $sqlAutores = "SELECT * FROM Autores WHERE autor LIKE ? ORDER BY autor ASC";
    $stmtAutores = $conn->prepare($sqlAutores);
    $likeBusqueda = "%" . $busqueda . "%";
    $stmtAutores->bind_param("s", $likeBusqueda);
    $stmtAutores->execute();
    $result = $stmtAutores->get_result();
} else {
    $sqlAutores = "SELECT * FROM Autores ORDER BY autor ASC";
    $result = $conn->query($sqlAutores);
}
?>
...
<main class="main">
    <div class="header-bar">
        ...
        <form class="search-form" method="GET" action="autores.php">
            <input 
                type="text" 
                name="busqueda" 
        ...
        <button class="btn-add-author" onclick="location.href='añadirautor.php'">
            <i class="fa fa-plus"></i> Añadir Autor
        </button>
    </div>
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-info" onclick="location.href='detalleautor.php?id=<?php echo $row['id_autor']; ?>'">
                                    Ver detalles
                                </button>
                                <button class="btn btn-danger" onclick="if(confirm('¿Seguro que deseas eliminar este autor?')) location.href='eliminarautor.php?id=<?php echo $row['id_autor']; ?>'">
                                    Eliminar
                                </button>
                            </td>
                        </tr>
