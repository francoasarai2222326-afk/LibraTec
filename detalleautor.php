...
// Procesar formulario de actualización de autor
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $nuevo_nombre = trim($_POST['autor']);
    if ($nuevo_nombre !== '') {
        $sqlUpdate = "UPDATE Autores SET autor = ? WHERE id_autor = ?";
        $stmtUpdate = $conn->prepare($sqlUpdate);
        $stmtUpdate->bind_param("si", $nuevo_nombre, $id_autor);
        if ($stmtUpdate->execute()) {
            // Recargar para mostrar cambios
            header("Location: detalleautor.php?id=" . $id_autor . "&msg=actualizado");
            exit();
        } else {
            $errorMsg = "Error al actualizar el autor.";
        }
    } else {
        $errorMsg = "El nombre del autor no puede estar vacío.";
    }
}

// Obtener datos del autor
$sqlAutor = "SELECT * FROM Autores WHERE id_autor = ?";
...
$sqlLibros = "SELECT * FROM Libros WHERE autor = ? ORDER BY titulo ASC";
$stmtLibros = $conn->prepare($sqlLibros);
$stmtLibros->bind_param("s", $autor['autor']);
$stmtLibros->execute();
...
<title>Detalle Autor - LIBRATEC</title>
...
<style>
    ...
    * {
    ...
    form.update-author input[type="text"] {
        width: 100%;
        padding: 10px 12px;
        font-size: 1.1rem;
        border-radius: 6px;
    ...
    form.update-author button:hover {
...
<main class="main">
    ...
    <?php if (isset($_GET['msg']) && $_GET['msg'] === 'actualizado'): ?>
        <p class="success-message">Autor actualizado correctamente.</p>
    <?php endif; ?>
    ...
    <form method="POST" class="update-author" action="detalleautor.php?id=<?php echo $id_autor; ?>">
        <label for="autor"><strong>Nombre del Autor:</strong></label>
        <input type="text" id="autor" name="autor" value="<?php echo htmlspecialchars($autor['autor']); ?>" required />
        <button type="submit">Actualizar Autor</button>
    </form>

    <h2>Libros del Autor "<?php echo htmlspecialchars($autor['autor']); ?>"</h2>
        ...
        <table>
            ...
            <tbody>
                <?php if ($resLibros && $resLibros->num_rows > 0): ?>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="10">Este autor no tiene libros registrados.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
