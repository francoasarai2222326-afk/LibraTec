...
// Obtener datos del alumno para el formulario
...
<form action="procesaractualizarinfoestudiante.php" method="POST">

    <h2>Actualizar Información</h2>

    <input type="hidden" name="no_control" value="<?php echo $alumno['no_control']; ?>">

    <label>Nombre</label>
    <input type="text" name="nombre" value="<?php echo $alumno['nombre']; ?>" required>

    <label>Apellido Paterno</label>
    <input type="text" name="apellido_paterno" value="<?php echo $alumno['apellido_paterno']; ?>" required>

    <label>Apellido Materno</label>
    <input type="text" name="apellido_materno" value="<?php echo $alumno['apellido_materno']; ?>" required>

    <label>Correo</label>
    <input type="email" name="correo" value="<?php echo $alumno['correo']; ?>" required>

    <label>Teléfono</label>
    <input type="text" name="telefono" value="<?php echo $alumno['telefono']; ?>" required>
    ...
    <select name="carrera" required>
        ...
        foreach ($carreras as $c) {
            $selected = ($alumno['carrera'] == $c) ? "selected" : "";
            echo "<option value='$c' $selected>$c</option>";
        }
        ?>
    </select>

    <label>Contraseña (dejar en blanco si no se quiere cambiar)</label>
    <input type="password" name="contrasena">

    <button type="submit">Guardar Cambios</button>
</form>

<a href="miperfilestudiante.php" class="back">← Regresar a mi perfil</a>
