...
// Consulta géneros con filtro búsqueda
if ($busqueda !== "") {
    $sqlGeneros = "SELECT * FROM Generos WHERE genero LIKE ? ORDER BY genero ASC";
    $stmtGeneros = $conn->prepare($sqlGeneros);
    $likeBusqueda = "%" . $busqueda . "%";
    $stmtGeneros->bind_param("s", $likeBusqueda);
    $stmtGeneros->execute();
    $result = $stmtGeneros->get_result();
} else {
    $sqlGeneros = "SELECT * FROM Generos ORDER BY genero ASC";
...
<main class="main">
    <div class="header-bar">
        ...
        <form class="search-form" method="GET" action="generos.php">
            <input 
                type="text" 
                name="busqueda" 
        ...
        <button class="btn-add-genero" onclick="location.href='añadirgenero.php'">
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-info" onclick="location.href='detallegenero.php?id=<?php echo $row['id_genero']; ?>'">
                                    Ver detalles
                                </button>
                                <button class="btn btn-danger" onclick="if(confirm('¿Seguro que deseas eliminar este género?')) location.href='eliminargenero.php?id=<?php echo $row['id_genero']; ?>'">
                                    Eliminar
                                </button>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="2">No se encontraron géneros.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
