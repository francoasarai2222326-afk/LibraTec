...
// Consulta carreras con filtro búsqueda
if ($busqueda !== "") {
    $sqlCarreras = "SELECT * FROM Carreras WHERE carrera LIKE ? ORDER BY carrera ASC";
    $stmtCarreras = $conn->prepare($sqlCarreras);
    $likeBusqueda = "%" . $busqueda . "%";
    $stmtCarreras->bind_param("s", $likeBusqueda);
    $stmtCarreras->execute();
    $result = $stmtCarreras->get_result();
} else {
    $sqlCarreras = "SELECT * FROM Carreras ORDER BY carrera ASC";
    $result = $conn->query($sqlCarreras);
}
?>
...
<main class="main">
    <div class="header-bar">
        ...
        <form class="search-form" method="GET" action="carreras.php">
            <input 
                type="text" 
                name="busqueda" 
        ...
        <button class="btn-add-carrera" onclick="location.href='añadircarrera.php'">
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-info" onclick="location.href='detallecarrera.php?id=<?php echo $row['id_carrera']; ?>'">
                                    Ver detalles
                                </button>
                                <button class="btn btn-danger" onclick="if(confirm('¿Seguro que deseas eliminar esta carrera?')) location.href='eliminarcarrera.php?id=<?php echo $row['id_carrera']; ?>'">
                                    Eliminar
                                </button>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="2">No se encontraron carreras.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
