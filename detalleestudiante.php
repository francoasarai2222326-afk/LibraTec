...
if (!isset($_GET['id_alumno'])) {
    echo "Error: No se recibió id_alumno.";
    exit();
}
...
$sqlAlumno = "SELECT * FROM Alumnos WHERE id_alumno = ? LIMIT 1";
...
$sqlPrestamos = "
    SELECT p.codigo, l.titulo, p.fecha_prestamo, p.fecha_devolucion, p.fecha_real_devolucion, p.estado, p.observaciones
    FROM prestamos p
    INNER JOIN libros l ON p.isbn = l.isbn
    WHERE p.no_control = ?
";
$stmtPrestamos = $conn->prepare($sqlPrestamos);
$stmtPrestamos->bind_param("s", $alumno['no_control']);
$stmtPrestamos->execute();
$resPrestamos = $stmtPrestamos->get_result();

// Consultar multas del alumno (con info de préstamo)
$sqlMultas = "
    SELECT 
        m.codigo, l.titulo, l.isbn,
        p.fecha_prestamo, p.fecha_devolucion, p.fecha_real_devolucion, p.estado AS estado_prestamo, p.observaciones AS observaciones_prestamo,
...
$stmtMultas->bind_param("s", $alumno['no_control']);
$stmtMultas->execute();
...
<title>Detalle del Alumno - LIBRATEC</title>
<style>
    * {
    ...
    .container {
        max-width: 900px;
        margin: 100px auto 40px; /* para que no tape el navbar fijo */
        background: white;
        border-radius: 12px;
        padding: 20px 30px;
    ...
    table {
        ...
        width: 100%;
        min-width: 900px; /* para que no se achique demasiado */
        table-layout: fixed;
        font-size: 0.95rem;
    }
    th, td {
